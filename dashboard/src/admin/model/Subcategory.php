...
class Subcategory
{
    ...
    // delete
    public function deleteSubcategory($id, $cate_id)
    {
        $this->con = Database::connect();
        $sql_for_valid = "SELECT count(*) as total FROM products WHERE subcategory_id = :id AND category_id = :cate_id";
        $statement_for_valid = $this->con->prepare($sql_for_valid);
        $statement_for_valid->bindParam(":id", $id);
        $statement_for_valid->bindParam(":cate_id", $cate_id);
